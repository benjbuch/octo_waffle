...
If you have not done so yet, please attach the `tidyverse` and make sure you have the `plate_data` object from the [last section](part_11-tidying_tables.md)--exercises included. If not, here you go.
...
A common summary of continuous data is to give averages and measure the spread e.g. using the standard deviation `sd(x)`, the median absolute deviation `mad(x)`, or the inter-quartile range `IQR(x)`. Given `plate_data`, let's calculate the average intensity for each `sample_id` and `concentration`.
