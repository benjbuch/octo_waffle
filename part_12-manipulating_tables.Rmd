...
If we want to explicitly specify arguments by name, we have to wrap the mapping call as formula with `~` and can use `..1` (or `.x`) and `..2` (or `.y`) to refer to the first and respectivley second input list.
...
If the entire table has _the exact same number and the exact same column names_ as the input arguments of the function, this simple `purrr::pmap` syntax works, irrespective of the column order.
...
What to do with this fancy new column containing lists? We could apply another function on them.
...
Another useful application is to calculate the row-wise sum, mean, median, standard deviation etc. even on a subselection of columns.
