...
To extract a substring from a string, base R has a function called `substr`. 
...
> If you need to match a point, you need to use an ‘escape’ to tell the regular expression not to use the special meaning of “`.`”, but its literal meaning. The ‘escape’ sign for regular expressions is `\`, so you regex `\.`. However, `\` is also the escape sign for R. In order to _perserve_ the escape sign for the evaluation of the regular expression, you need to escape the escape sign as well. (Sounds confusing.) 
...
By default, these matches are ‘greedy’. They will match the _longest_ string possible. To make them match the _shortest_ string possible, put a `?` after them.
