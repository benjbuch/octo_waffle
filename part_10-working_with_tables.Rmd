...
There are (at least) three different flavours to work with a `data.frame` in R:
...
Of course, the sample assignments had been documented (somewhere). So, we know that _the actual data_ should have been annotated like this.
...
> If each variable forms a column, each row represents a single observation, and each cell represents a single value, we refer to this as **tidy data**. 
...
Typically, if the table with your data is wider than long (if it is in the ‘wide format’), it's easy to encounter messy data. For example, `plate_1` is in one (of many possible) wide and messy formats: The concentration parameter is stored along the header row, not in a separate column. 
...
For your convenience, this is an overview of the different functions that have been developed in the past to interconvert wide and long table formats. (In case you encounter one of these or need help on related topics.)
...
Let's next spread the gathered representation of `plate_1` into a wide format! Maybe, we would like to see the measured intensities for `control` and `treatment_A` side-by-side given each concentration and the replicate.
...
plate_1 %>% 
  ...
  spread(key   = sample_id, # names (unquoted) of columns to use as column headers
         value = intensity, # names (unquoted) of columns to populate the table body
         ) %>% 
  # print only top rows
  head(3) 
```
...
```{r}
plate_1 %>% unite(col = "experiment",      # name of the new column
                  sample_id, replicate_id, # names (unquoted) of the columns to paste
...
The `tidyr::separate(...)` function does the reverse operation. By default, any non-alphanumeric character will be used to split the column. Note that missing pieces will be replaced with `NA`.

```{r}
plate_1 %>% separate(col  = sample_id,                 # name (can be unquoted)
...
sample_id + replicate_id. how can this be useful? spreading multiple parameters (not possible in tidyr)